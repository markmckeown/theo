...
Ensure(test_cell_body_add_entry)
{
	...
	assert_equal(cell_dir_get_chunk(&cell.cell_header->cell_dir,
					(char *)cell.cell_header, &checksum,
					&out), true);
	assert_equal(out.checksum.bytes[3], 23);
	assert_equal(out.offset, 0);
...
Ensure(test_cell_body_add_entries)
{
	...
	assert_equal(cell.cell_header->cell_body.next_entry_offset,
		     2 * sizeof(struct cell_body_entry) +
		     strlen(another_string) + strlen(string));
	assert_equal(cell.cell_header->cell_body.top_entry_offset,
		     2 * sizeof(struct cell_body_entry) +
		     strlen(another_string) + strlen(string));
	xfree(buffer);
...
Ensure(test_cell_body_add_entries_offset)
{
	...
	cell_body_entry =
	    (struct cell_body_entry *)(cell.slab +
				       sizeof(struct cell_body_entry) +
				       strlen(third_string));
	assert_equal(cell_body_entry->free_space, 1);
	assert_equal(cell_body_entry->top_entry, 0);
	assert_equal(cell_body_entry->size,
		     (124 + sizeof(struct cell_body_entry)) -
		     (2 * sizeof(struct cell_body_entry) +
		      strlen(third_string)));
...
Ensure(test_cell_body_add_entries_offset_overflow)
{
	...
	for (i = 0; i < 5; i++) {
		...
		cell_body_add(&cell.cell_header->cell_body, &cell, &checksum,
			      string, 10, &out);
		assert_equal(out.offset,
			     i * (sizeof(struct cell_body_entry) + 10));
		assert_equal(cell.cell_header->cell_body.next_entry_offset,
...
Ensure(test_cell_body_add_entries_offset_small_buffer)
{
	...
	slab_size =
	    sizeof(struct cell_header) + 5 * (sizeof(struct cell_body_entry) +
					      10) + 5 +
...
Ensure(test_cell_body_add_entries_small_buffer_header_pushdown)
{
	...
	assert_equal(cell.cell_header->cell_body.next_entry_offset,
		     (i + 1) * (sizeof(struct cell_body_entry) + 10));
	assert_equal(cell.cell_header->cell_body.top_entry_offset, top);
	i++;
...
Ensure(test_cell_body_add_entries_small_buffer_header_pushdown_free)
{
	struct cell cell;
	char *buffer;
	struct cell_dir_entry out;
	...
	//Empty some space at the top
	cell.cell_header->cell_body.next_entry_offset =
	    3 * (sizeof(struct cell_body_entry) + 10);
	checksum.bytes[17] = i++;
	cell_body_add(&cell.cell_header->cell_body, &cell, &checksum,
		      string_fifteen, 15, &out);
	assert_equal(cell.cell_header->cell_body.top_entry_offset,
		     3 * (sizeof(struct cell_body_entry) + 10) +
		     (sizeof(struct cell_body_entry) + 15));
	cell.cell_header->cell_body.next_entry_offset = 0;
...
Ensure(test_cell_body_add_entries_top_full)
{
	...
	cell.cell_header->cell_body.top_entry_offset =
	    cell.slab_size - (sizeof(struct cell_header) +
	...
	assert_equal(cell_body_entry->size,
		     (124 + sizeof(struct cell_body_entry)) -
		     (2 * sizeof(struct cell_body_entry) + 86));
...
TestSuite *cell_body_tests()
{
	...
	add_test(suite,
		 test_cell_body_add_entries_small_buffer_header_pushdown);
	add_test(suite,
		 test_cell_body_add_entries_small_buffer_header_pushdown_free);
