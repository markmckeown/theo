...
int chunker_chunk_buffer(struct chunker *chunker, 
	...
	rolling_hash2_reset(&chunker->rh_state2, 
			(uint8_t *) (buffer + (CHUNKER_MIN_CHUNK_SIZE - CHUNKER_WINDOW_WIDTH)));
	...
	ret_t = rolling_hash2_run(&chunker->rh_state2, (uint8_t *) buffer, max_len,
		      CHUNKER_MASK, CHUNKER_TRIGGER, &offset);
	if (ret_t == FINGERPRINT_RET_HIT) {
		chunk_ref_set(chunk_ref, buffer, CHUNKER_MIN_CHUNK_SIZE + offset);
		ret = CHUNKER_FOUND;
		goto out;
	}	
